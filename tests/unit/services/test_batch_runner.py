...
class TestBatchRunner:
    ...
    def test_submit_runs_jobs_and_calls_callbacks(self, mock_pipeline_cls, tmp_path) -> None:
        ...
        def mock_run(input_path, output_dir, preset, preprocess_config=None, trace_params=None, on_progress=None):
            if on_progress:
        ...
        assert any(p.job_id == "job_1" and p.step == "loading" and p.percent == 10 for p in progress_updates)
        assert any(p.job_id == "job_2" and p.step == "done" and p.percent == 100 for p in progress_updates)
