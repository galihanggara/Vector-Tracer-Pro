...
def test_cleanup_on_exception(bitmapper: Bitmapper):
    ...
    meta = ImageMetadata(width=10, height=10, dpi=(72.0, 72.0), bit_depth=1, original_mode="L")
    processed = ProcessedImage(
        data=data, metadata=meta, category=ImageCategory.LINE_ART, config=PreprocessConfig()
    )

    temp_path = None
    with pytest.raises(RuntimeError), bitmapper.write(processed, BitmapFormat.PBM) as bitmap:
...
def test_validation_errors(bitmapper: Bitmapper):
    ...
    p_3d = ProcessedImage(
        data=data_3d, metadata=meta, category=ImageCategory.LINE_ART, config=PreprocessConfig()
    )
    with pytest.raises(ValueError) as exc, bitmapper.write(p_3d, BitmapFormat.PBM):
        pass
    ...
    p_u8 = ProcessedImage(
        data=data_u8, metadata=meta, category=ImageCategory.LINE_ART, config=PreprocessConfig()
    )  # type: ignore
    with pytest.raises(ValueError) as exc, bitmapper.write(p_u8, BitmapFormat.PBM):
        pass
    ...
    p_empty = ProcessedImage(
        data=data_empty, metadata=meta, category=ImageCategory.LINE_ART, config=PreprocessConfig()
    )
    with pytest.raises(ValueError) as exc, bitmapper.write(p_empty, BitmapFormat.PBM):
        pass
