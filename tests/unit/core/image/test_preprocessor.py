...
def test_preprocess_flat_vector(
    loader: ImageLoader, preprocessor: Preprocessor, fixtures_dir: Path
):
    img_data = loader.load(fixtures_dir / "flat_vector.png")

    # Process FLAT_VECTOR with skip_denoise=True so smooth average filter
    # doesn't introduce extra colors
...
def test_preprocess_default_config_all_categories(
    loader: ImageLoader, preprocessor: Preprocessor, fixtures_dir: Path
):
